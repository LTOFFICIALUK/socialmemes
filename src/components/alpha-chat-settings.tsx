...
export const AlphaChatSettings = ({ onClose }: AlphaChatSettingsProps) => {
  ...
  const handleToggle = async () => {
    try {
      ...
      console.error('Error toggling alpha chat:', error)
      alert('Failed to update alpha chat settings. Please try again.')
    } finally {
      setIsToggling(false)
    }
  }
          ...
          {isEnabled && (
            <div className="bg-black border border-green-500/20 rounded-lg p-4">
              ...
              <ul className="text-sm text-gray-300 space-y-1">
                ...
                <li>• Subscription pricing: 0.1-0.8 SOL per month</li>
              </ul>
            </div>
          )}
